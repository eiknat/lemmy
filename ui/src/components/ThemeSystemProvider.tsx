...
function ThemeSystemProvider({ children }) {
  ...
  const theme = Object.keys(themes).includes(currentTheme) ? themes[currentTheme] : themes.dark;
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener('change-theme', handleThemeChange);
  }, []);
